...
// Mirrors src/server.js exactly, but writes each route's response to a
// static .json file instead of serving it live. Re-run after editing the DB.
const TITLE_LINEAGE = `CASE WHEN winner IN ('West Germany', 'Germany') THEN 'Germany' ELSE winner END`;

function buildEditionsList() {
  return all(`
...
function buildStats() {
  ...
  `).n;
  return { editions, matches, goals, countries };
}

function buildEditionDetail(year) {
  const edition = get(`
    SELECT * FROM (
      SELECT *, COUNT(*) OVER (PARTITION BY ${TITLE_LINEAGE} ORDER BY year) AS title_number
      FROM editions
  ...
  return { edition, awards, matches: matchesOut };
}

function buildMatchDetail(id) {
  ...
  return { ...match, penaltyKicks: kicks.length ? penaltyKicks : null, goals, cards };
}
...
const editions = buildEditionsList();
write('editions.json', editions);
write('stats.json', buildStats());
write('records.json', buildRecords());

for (const e of editions) {
  write(`editions/${e.year}.json`, buildEditionDetail(e.year));
}

const matchIds = all('SELECT id FROM matches ORDER BY id').map(r => r.id);
for (const id of matchIds) {
  write(`matches/${id}.json`, buildMatchDetail(id));
}

console.log(`Exported ${editions.length} editions, ${matchIds.length} matches to ${OUT_DIR}`);
